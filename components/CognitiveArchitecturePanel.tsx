...
export const CognitiveArchitecturePanel = React.memo(() => {
    ...
    const renderSynthesizedSkill = (skill: SynthesizedSkill) => (
        <div key={skill.id} className={`arch-module synthesized ${skill.status === 'deprecated' ? 'deprecated' : ''}`}>
            <span className={`arch-status-indicator status-${skill.status === 'deprecated' ? 'inactive' : 'active'}`} title={`Synthesized Skill (${skill.status})`}></span>
            <span className="arch-module-name">{skill.name}{skill.status === 'deprecated' && ' (deprecated)'}</span>
            <span className="arch-module-version" title={`${skill.steps.length} steps`}>Synth</span>
        </div>
    );
            ...
            <div className="cognitive-arch-content">
                ...
                {synthesizedSkills && synthesizedSkills.length > 0 && (
                    <div className="arch-group">
                        <div className="arch-group-header">Synthesized Skills</div>
